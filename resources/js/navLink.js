document.addEventListener('DOMContentLoaded', function () {
  ...
  function setActiveLink() {
    ...
    sideNavLinks.forEach(link => {
      link.classList.remove('!text-blue-600', 'font-medium', 'bg-blue-500/5', 'hover:bg-blue-500/5');
    });
    ...
    if (currentHash) {
      ...
      if (activeDefaultLink) {
        activeDefaultLink.classList.add('!text-blue-600');
      }

      const activeSideLink = document.querySelector(`.side-nav-link[href$="${currentHash}"]`);
      if (activeSideLink) {
        activeSideLink.classList.add('!text-blue-600', 'font-medium', 'bg-blue-500/5', 'hover:bg-blue-500/5');
      }
    }
    ...
    sideNavLinks.forEach(link => {
      if (link.href === currentUrl) {
        link.classList.add('!text-blue-600', 'font-medium', 'bg-blue-500/5', 'hover:bg-blue-500/5');
      }
    });
  }

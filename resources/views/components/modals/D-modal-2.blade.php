<div id="D-modal-2"
  ...
  role="dialog" tabindex="-1" aria-labelledby="D-modal-2-label">
  <div
    ...
    <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
      ...
      <div class="flex justify-between items-center py-5 px-7 border-b border-blue-500/10">
        <h3 id="D-modal-2-label">
          Request for New ID
        </h3>
        <button type="button" class="btn btn-ghost text-gray-700 px-2.5" aria-label="Close" data-hs-overlay="#D-modal-2">
      ...
      <div class="p-7">
        <div class="grid md:grid-cols-2 grid-cols-1 gap-x-5 md:gap-y-6 gap-y-5">
          ...
          <!-- Precint number -->
          <div class="md:space-y-2.5 space-y-1.5">
            <label for="precint_number" class="input-label">Precint number <span class="text-red-500">*</span></label>
            <input type="text" name="precint_number" id="precint_number"
              class="default-input @error('precint_number') !border-red-500 !bg-red-500/5 @enderror"
              placeholder="E.g. 0090A" value="{{ old('precint_number') }}">
            @error('precint_number')
            <p style="color:red;font-size:0.8rem;margin-top:0.1px;">{{ $message }}</p>
            @enderror
          </div>
          <!-- Date of Claim -->
          ...
          <div class="md:col-span-2 col-span-1 flex justify-end gap-4 mt-6">
            <button aria-label="Close" data-hs-overlay="#D-modal-2" class="btn btn-secondary">Cancel</button>
